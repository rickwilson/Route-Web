...
<head>
    ...
    <script type="text/javascript">
        this.globalUrl = '/client/report/shipping';
    ...
    <div class="page-content">
        <div class="content">
            <div class="page-title">
                <h3>Reports - <span class="semi-bold">Shipping</span></h3>
            </div>
            <div class="row-fluid">
                <div class="span12">
                    <div class="grid simple ">
                        <div class="grid-title">
                            ...
                            <%--<select name="tagInput" id="tagInput" onchange="changeShipTag();">--%>
                                <%--<option value="1"<c:if test="${tag == '1'}"> SELECTED</c:if>>All Ship Status</option>--%>
                                <%--<option value="2"<c:if test="${tag == '2'}"> SELECTED</c:if>>Delivered</option>--%>
                                <%--<option value="3"<c:if test="${tag == '3'}"> SELECTED</c:if>>In Transit</option>--%>
                                <%--<option value="4"<c:if test="${tag == '4'}"> SELECTED</c:if>>Ready</option>--%>
                                <%--<option value="5"<c:if test="${tag == '5'}"> SELECTED</c:if>>Exception</option>--%>
                                <%--<option value="6"<c:if test="${tag == '6'}"> SELECTED</c:if>>No Tracking #</option>--%>
                            <%--</select>--%>
                            <div class="tools hidden-xs">
                                ...
                                <select name="searchTypeInput" id="searchTypeInput" onchange="changeSearchType();">
                                    <option value="1"<c:if test="${searchType == '1'}"> SELECTED</c:if>>Tracking #</option>
                                    <option value="2"<c:if test="${searchType == '2'}"> SELECTED</c:if>>Courier</option>
                                    <option value="3"<c:if test="${searchType == '3'}"> SELECTED</c:if>>In OrderId</option>
                                    <option value="4"<c:if test="${searchType == '4'}"> SELECTED</c:if>>In First</option>
                                    <option value="5"<c:if test="${searchType == '5'}"> SELECTED</c:if>>In Last</option>
                                    <option value="6"<c:if test="${searchType == '6'}"> SELECTED</c:if>>In Province</option>
                                    <option value="7"<c:if test="${searchType == '7'}"> SELECTED</c:if>>In Country</option>
                                </select>
                            </div>
                        </div>
                        <div class="grid-body">
                            <table class="table table-striped" >
                                ...
                                <tr>
                                    ...
                                    <th><a href="javascript:void(0);" onclick="changeSort(2);">Tracking #</a></th>
                                    <th><a href="javascript:void(0);" onclick="changeSort(3);">Courier</a></th>
                                    <th><a href="javascript:void(0);" onclick="changeSort(4);">Status</a></th>
                                    <th><a href="javascript:void(0);" onclick="changeSort(5);">Order ID</a></th>
                                    <th><a href="javascript:void(0);" onclick="changeSort(6);">Customer</a></th>
                                    <th><a href="javascript:void(0);" onclick="changeSort(7);">Destination</a></th>
                                </tr>
                                </thead>
                                <tbody>
                                <c:forEach var="clientReportItemObject" items="${clientReportItemObjects}">
                                    <tr>
                                        ...
                                        <td><a href="/client/transactionDetail?tId=${clientReportItemObject.transactionId}"><c:out value="${clientReportItemObject.trackingNumber}" /></a></td>
                                        <td><c:out value="${clientReportItemObject.courier}" /></td>
                                        <td><c:out value="${clientReportItemObject.shipStatusTagFormatted}" /></td>
                                        <td><c:out value="${clientReportItemObject.orderId}" /></td>
                                        <td><c:out value="${clientReportItemObject.customerName}" /></td>
                                        <td><c:out value="${clientReportItemObject.destination}" /></td>
                                    </tr>
                                </c:forEach>
                                </tbody>
                            </table>
